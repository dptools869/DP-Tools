...
import { bmpToSvg, BmpToSvgOutput } from '@/ai/flows/bmp-to-svg';
import AdBanner from '@/components/ad-banner';

export default function BmpToSvgPage() {
  const [file, setFile] = useState<File | null>(null);
  const [isConverting, setIsConverting] = useState(false);
  const [conversionResult, setConversionResult] = useState<BmpToSvgOutput | null>(null);
  ...
  const convertFile = async () => {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        try {
          const result = await bmpToSvg({ bmpDataUri: base64File, fileName: file.name });
        ...
        <main className="lg:col-span-3">
          <Card className="shadow-2xl shadow-primary/10 border-primary/20 bg-card/80 backdrop-blur-sm mb-12 hidden md:block">
            <CardHeader className="text-center">
              ...
              <CardTitle className="text-3xl font-headline">BMP to SVG Converter</CardTitle>
              <CardDescription className="text-lg">
                Effortlessly convert your BMP images into scalable SVG vector graphics.
              </CardDescription>
            </CardHeader>
          </Card>
          <Card>
            <CardHeader className="md:hidden">
                <CardTitle>BMP to SVG Converter</CardTitle>
            </CardHeader>
          ...
          <article className="mt-16 prose prose-lg dark:prose-invert max-w-none prose-h2:font-headline prose-h2:text-3xl prose-h2:text-primary prose-a:text-primary">
            <h2 id="about-tool">Convert BMP to Scalable Vector Graphics (SVG)</h2>
            <p>Our BMP to SVG converter provides a powerful way to convert your raster BMP images into scalable, editable SVG vector graphics. SVG is an XML-based format that allows for infinite scaling without loss of quality, making it ideal for logos, icons, and web graphics that need to look crisp on any screen size.</p>
            <AdBanner type="top-banner" className="my-8"/>
            <h2 id="how-it-works">How Does BMP to SVG Conversion Work?</h2>
            <p>The conversion process uses advanced tracing algorithms to analyze the pixels in your BMP and convert them into mathematical paths and shapes. The result is a clean SVG file that represents your original image in a vector format. We prioritize your privacy; all files are handled securely and are automatically deleted from our servers after processing.</p>
            ...
            <ul>
              ...
              <li><strong>Completely Free:</strong> Convert as many BMP files to SVG as you need, at no cost.</li>
            </ul>
            <p>Unlock the scalability of your images by converting them to SVG. Experience the simplicity and quality of our free online conversion tool today!</p>
          </article>
