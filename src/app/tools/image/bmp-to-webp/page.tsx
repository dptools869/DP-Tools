...
import { bmpToWebp, BmpToWebpOutput } from '@/ai/flows/bmp-to-webp';
import AdBanner from '@/components/ad-banner';

export default function BmpToWebpPage() {
  const [file, setFile] = useState<File | null>(null);
  const [isConverting, setIsConverting] = useState(false);
  const [conversionResult, setConversionResult] = useState<BmpToWebpOutput | null>(null);
  ...
  const convertFile = async () => {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        try {
          const result = await bmpToWebp({ bmpDataUri: base64File, fileName: file.name });
        ...
        <main className="lg:col-span-3">
          <Card className="shadow-2xl shadow-primary/10 border-primary/20 bg-card/80 backdrop-blur-sm mb-12 hidden md:block">
            <CardHeader className="text-center">
              ...
              <CardTitle className="text-3xl font-headline">BMP to WEBP Converter</CardTitle>
              <CardDescription className="text-lg">
                Effortlessly convert your BMP images into the modern, efficient WEBP format.
              </CardDescription>
            </CardHeader>
          </Card>
          <Card>
            <CardHeader className="md:hidden">
              <CardTitle>BMP to WEBP Converter</CardTitle>
            </CardHeader>
          ...
          <article className="mt-16 prose prose-lg dark:prose-invert max-w-none prose-h2:font-headline prose-h2:text-3xl prose-h2:text-primary prose-a:text-primary">
            <h2 id="about-tool">Convert BMP to Modern WEBP Images</h2>
            <p>Our BMP to WEBP converter is the perfect tool for converting uncompressed bitmap images into the highly efficient WEBP format. WEBP offers superior lossless and lossy compression for images on the web, resulting in smaller file sizes and faster load times without sacrificing quality.</p>
            <AdBanner type="top-banner" className="my-8"/>
            <h2 id="how-it-works">How Does BMP to WEBP Conversion Work?</h2>
            <p>The process is seamless. When you upload your BMP file, our advanced conversion engine reads the pixel data and re-encodes it into the WEBP format using modern compression algorithms. We prioritize your privacy; all files are handled securely and are automatically deleted from our servers after processing.</p>
            <h3 id="key-features">Key Features and Benefits</h3>
            <ul>
              <li><strong>Superior Compression:</strong> Create smaller image files compared to JPG and PNG, improving website performance.</li>
              <li><strong>High Quality:</strong> WEBP provides excellent image quality for both lossy and lossless compression.</li>
              ...
              <li><strong>Completely Free:</strong> Convert as many BMP files to WEBP as you need, at no cost.</li>
            </ul>
            <p>Optimize your images for the web by converting them to the WEBP format. Experience the simplicity and quality of our free online conversion tool today!</p>
          </article>
