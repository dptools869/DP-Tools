...
import { aiToPng, AiToPngOutput } from '@/ai/flows/ai-to-png';
...
export default function AiToPngPage() {
  ...
  const [conversionResult, setConversionResult] = useState<AiToPngOutput | null>(null);
  ...
  const convertFile = async () => {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        try {
          const result = await aiToPng({ aiDataUri: base64File, fileName: file.name });
          setConversionResult(result);
          toast({
            title: 'Conversion Successful',
            description: 'Your PNG file is ready for download.',
          });
        } catch (error) {
           toast({
            variant: 'destructive',
  ...
  const downloadPng = () => {
    if (conversionResult) {
      ...
      link.href = conversionResult.pngDataUri;
        ...
        <main className="lg:col-span-3">
          <Card className="shadow-2xl shadow-primary/10 border-primary/20 bg-card/80 backdrop-blur-sm mb-12 hidden md:block">
            <CardHeader className="text-center">
              ...
              <CardTitle className="text-3xl font-headline">AI to PNG Converter</CardTitle>
              <CardDescription className="text-lg">
                Convert your Adobe Illustrator (.ai) files to high-quality PNG images.
              </CardDescription>
            </CardHeader>
          </Card>
          <Card>
            <CardHeader className="md:hidden">
                <CardTitle>AI to PNG Converter</CardTitle>
            </CardHeader>
                    ...
                    <AlertDescription className="flex flex-col sm:flex-row justify-between items-center gap-4 mt-4">
                       <p className="text-center sm:text-left">Your file <span className="font-semibold">{conversionResult.fileName}</span> is ready.</p>
                        <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                             <Button onClick={downloadPng} size="sm" className="bg-primary hover:bg-primary/90">
          ...
          <article className="mt-16 prose prose-lg dark:prose-invert max-w-none prose-h2:font-headline prose-h2:text-3xl prose-h2:text-primary prose-a:text-primary">
            <h2 id="about-tool">Convert Adobe Illustrator (AI) to PNG</h2>
            <p>Our AI to PNG converter provides a simple and reliable way to turn your Adobe Illustrator vector graphics into high-quality, lossless PNG images. This is essential for web designers, marketers, and anyone who needs to use vector assets in a raster format that supports transparency.</p>
            <AdBanner type="top-banner" className="my-8"/>
            <h2 id="how-it-works">How Does AI to PNG Conversion Work?</h2>
            <p>The process is simple. When you upload your AI file, our system uses an advanced conversion engine to render your vector graphic into a high-resolution PNG image. The layout and vector data are preserved, so your image appears just as it did in the original file. We prioritize your privacy; all files are handled securely and are automatically deleted from our servers after processing.</p>
            ...
            <ul>
              ...
              <li><strong>Transparency Support:</strong> PNG is the ideal format for preserving transparent backgrounds from your AI files.</li>
              ...
              <li><strong>Completely Free:</strong> Convert as many AI files to PNG as you need, at no cost.</li>
            </ul>
            <p>Make your vector graphics web-ready and universally viewable by converting them to PNG. Experience the simplicity and quality of our free online conversion tool today!</p>
          </article>
