...
import { bmpToPng, BmpToPngOutput } from '@/ai/flows/bmp-to-png';
import AdBanner from '@/components/ad-banner';

export default function BmpToPngPage() {
  const [file, setFile] = useState<File | null>(null);
  const [isConverting, setIsConverting] = useState(false);
  const [conversionResult, setConversionResult] = useState<BmpToPngOutput | null>(null);
  ...
  const convertFile = async () => {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        try {
          const result = await bmpToPng({ bmpDataUri: base64File, fileName: file.name });
        ...
        <main className="lg:col-span-3">
          <Card className="shadow-2xl shadow-primary/10 border-primary/20 bg-card/80 backdrop-blur-sm mb-12 hidden md:block">
            <CardHeader className="text-center">
              ...
              <CardTitle className="text-3xl font-headline">BMP to PNG Converter</CardTitle>
              <CardDescription className="text-lg">
                Effortlessly convert your BMP image files into high-quality PNGs.
              </CardDescription>
            </CardHeader>
          </Card>
          <Card>
          ...
          <article className="mt-16 prose prose-lg dark:prose-invert max-w-none prose-h2:font-headline prose-h2:text-3xl prose-h2:text-primary prose-a:text-primary">
            <h2 id="about-tool">Convert BMP to High-Quality PNG</h2>
            <p>Our BMP to PNG converter is the perfect tool for converting uncompressed bitmap images into the versatile and high-quality PNG format. PNG (Portable Network Graphics) is a lossless format that supports transparency, making it ideal for web graphics, logos, and images where clarity is essential.</p>
            <AdBanner type="top-banner" className="my-8"/>
            <h2 id="how-it-works">How Does BMP to PNG Conversion Work?</h2>
            <p>The process is seamless. When you upload your BMP file, our advanced conversion engine reads the pixel data and re-encodes it into the PNG format. The lossless nature of PNG ensures that no quality is lost during this process. We prioritize your privacy; all files are handled securely and are automatically deleted from our servers after processing.</p>
            <h3 id="key-features">Key Features and Benefits</h3>
            <ul>
              <li><strong>Lossless Conversion:</strong> Retains the full quality of your original bitmap image.</li>
              <li><strong>Transparency Support:</strong> PNG is the ideal format if you need a transparent background.</li>
              ...
              <li><strong>Completely Free:</strong> Convert as many BMP files to PNG as you need, at no cost.</li>
            </ul>
            <p>Upgrade your BMP images to the more modern and flexible PNG format. Experience the simplicity and quality of our free online conversion tool today!</p>
          </article>
