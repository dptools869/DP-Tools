...
import { gifToJpg, GifToJpgOutput } from '@/ai/flows/gif-to-jpg';
import AdBanner from '@/components/ad-banner';

export default function GifToJpgPage() {
  const [file, setFile] = useState<File | null>(null);
  const [isConverting, setIsConverting] = useState(false);
  const [conversionResult, setConversionResult] = useState<GifToJpgOutput | null>(null);
    ...
    if (selectedFile) {
      if (selectedFile.type === 'image/gif') {
        ...
        toast({
          ...
          description: 'Please upload a GIF file (.gif).',
        });
        event.target.value = ''; 
      }
    }
  ...
  const handleDrop = (event: React.DragEvent<HTMLLabelElement>) => {
    ...
    if (droppedFile && droppedFile.type === 'image/gif') {
      ...
      toast({
        variant: 'destructive',
        title: 'Invalid File Type',
        description: 'Please drop a GIF file (.gif).',
      });
    }
  };
  ...
  const convertFile = async () => {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        try {
          const result = await gifToJpg({ gifDataUri: base64File, fileName: file.name });
        ...
        <main className="lg:col-span-3">
          <Card className="shadow-2xl shadow-primary/10 border-primary/20 bg-card/80 backdrop-blur-sm mb-12 hidden md:block">
            <CardHeader className="text-center">
              ...
              <CardTitle className="text-3xl font-headline">GIF to JPG Converter</CardTitle>
                  ...
                  >
                    <div className="flex flex-col items-center space-y-4">
                        ...
                        <span className="text-lg font-medium text-foreground">
                            {fileName || 'Drag & drop your GIF file here'}
                        </span>
                        <span className="text-muted-foreground">or click to browse</span>
                    </div>
                    <Input id="file-upload" type="file" className="sr-only" onChange={handleFileChange} accept="image/gif" disabled={isConverting} />
                  </Label>
          ...
          <article className="mt-16 prose prose-lg dark:prose-invert max-w-none prose-h2:font-headline prose-h2:text-3xl prose-h2:text-primary prose-a:text-primary">
            <h2 id="about-tool">Convert GIFs to Static JPG Images</h2>
            <p>Our GIF to JPG converter allows you to easily convert animated GIF files into standard, static JPG images. This is useful when you need to extract a frame from a GIF or use the image in a context where animation is not supported or desired. The conversion process typically captures the first frame of the animation to create the JPG.</p>
            <AdBanner type="top-banner" className="my-8"/>
            <h2 id="how-it-works">How Does GIF to JPG Conversion Work?</h2>
            <p>The process is straightforward. Upload your GIF file, and our system will convert the image data into the JPG format, using lossy compression to create a small, web-friendly file. The animation is discarded, leaving you with a high-quality static image. Your privacy is important to us; all files are handled securely and are automatically deleted from our servers after processing.</p>
            ...
            <ul>
              ...
              <li><strong>Web-Friendly:</strong> JPG is a universal format, perfect for photos and web graphics where small file size is important.</li>
              ...
              <li><strong>Completely Free:</strong> Convert as many GIFs to JPGs as you need, at no cost.</li>
            </ul>
            <p>Make your animated GIFs more versatile by converting them to standard JPG images. Experience the simplicity and quality of our free online conversion tool today!</p>
          </article>
