...
import { aiToWebp, AiToWebpOutput } from '@/ai/flows/ai-to-webp';
import AdBanner from '@/components/ad-banner';

const acceptedMimeTypes = ['application/postscript', 'application/illustrator'];
const acceptedExtensions = ['.ai'];

export default function AiToWebpPage() {
  const [file, setFile] = useState<File | null>(null);
  const [isConverting, setIsConverting] = useState(false);
  const [conversionResult, setConversionResult] = useState<AiToWebpOutput | null>(null);
  const [fileName, setFileName] = useState('');
  const { toast } = useToast();

  const isFileValid = (file: File) => {
    const fileExtension = '.' + file.name.split('.').pop()?.toLowerCase();
    return acceptedExtensions.includes(fileExtension) || acceptedMimeTypes.includes(file.type);
  }
    ...
    if (selectedFile) {
      if (isFileValid(selectedFile)) {
        ...
        toast({
          ...
          description: 'Please upload an Adobe Illustrator (.ai) file.',
        });
        event.target.value = ''; 
      }
    }
  ...
  const handleDrop = (event: React.DragEvent<HTMLLabelElement>) => {
    ...
    if (droppedFile && isFileValid(droppedFile)) {
      ...
      toast({
        variant: 'destructive',
        title: 'Invalid File Type',
        description: 'Please drop an Adobe Illustrator (.ai) file.',
      });
    }
  };
  ...
  const convertFile = async () => {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        try {
          const result = await aiToWebp({ aiDataUri: base64File, fileName: file.name });
          setConversionResult(result);
          toast({
            title: 'Conversion Successful',
            description: 'Your WEBP file is ready for download.',
          });
        } catch (error) {
           toast({
            variant: 'destructive',
  ...
  const downloadWebp = () => {
    if (conversionResult) {
      ...
      link.href = conversionResult.webpDataUri;
        ...
        <main className="lg:col-span-3">
          <Card className="shadow-2xl shadow-primary/10 border-primary/20 bg-card/80 backdrop-blur-sm mb-12 hidden md:block">
            <CardHeader className="text-center">
              ...
              <CardTitle className="text-3xl font-headline">AI to WEBP Converter</CardTitle>
              <CardDescription className="text-lg">
                Convert your Adobe Illustrator (.ai) files to modern WEBP images.
              </CardDescription>
            </CardHeader>
          </Card>
          <Card>
            <CardHeader className="md:hidden">
                <CardTitle>AI to WEBP Converter</CardTitle>
            </CardHeader>
                    ...
                    <AlertDescription className="flex flex-col sm:flex-row justify-between items-center gap-4 mt-4">
                       <p className="text-center sm:text-left">Your file <span className="font-semibold">{conversionResult.fileName}</span> is ready.</p>
                        <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                             <Button onClick={downloadWebp} size="sm" className="bg-primary hover:bg-primary/90">
          ...
          <article className="mt-16 prose prose-lg dark:prose-invert max-w-none prose-h2:font-headline prose-h2:text-3xl prose-h2:text-primary prose-a:text-primary">
            <h2 id="about-tool">Convert Adobe Illustrator (AI) to WEBP</h2>
            <p>Our AI to WEBP converter provides a simple and reliable way to turn your Adobe Illustrator vector graphics into modern, efficient WEBP images. WEBP is a next-generation format that provides superior lossless and lossy compression for images on the web, resulting in smaller file sizes and faster page loads.</p>
            <AdBanner type="top-banner" className="my-8"/>
            <h2 id="how-it-works">How Does AI to WEBP Conversion Work?</h2>
            <p>The process is simple. When you upload your AI file, our system uses an advanced conversion engine to render your vector graphic into a high-quality WEBP image. The layout and vector data are preserved, so your image appears just as it did in the original file. We prioritize your privacy; all files are handled securely and are automatically deleted from our servers after processing.</p>
            <h3 id="key-features">Key Features and Benefits</h3>
            <ul>
              <li><strong>High-Fidelity Conversion:</strong> Retains the original quality and detail of your vector graphics.</li>
              <li><strong>Optimized for Web:</strong> WEBP images are smaller than traditional formats, improving website performance.</li>
              ...
              <li><strong>Completely Free:</strong> Convert as many AI files to WEBP as you need, at no cost.</li>
            </ul>
            <p>Make your vector graphics web-ready and efficient by converting them to WEBP. Experience the simplicity and quality of our free online conversion tool today!</p>
          </article>
