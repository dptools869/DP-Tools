...
import { jpgToPdf, JpgToPdfOutput } from '@/ai/flows/jpg-to-pdf';
import AdBanner from '@/components/ad-banner';

export default function JpgToPdfPage() {
  const [file, setFile] = useState<File | null>(null);
  const [isConverting, setIsConverting] = useState(false);
  const [conversionResult, setConversionResult] = useState<JpgToPdfOutput | null>(null);
  ...
  const convertFile = async () => {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        try {
          const result = await jpgToPdf({ jpgDataUri: base64File, fileName: file.name });
        ...
        <main className="lg:col-span-3">
          ...
          <Card>
            <CardHeader className="md:hidden">
              <CardTitle>JPG/JPEG to PDF</CardTitle>
            </CardHeader>
              ...
              {!conversionResult && (
                <div className="space-y-6">
                  ...
                  >
                    ...
                    <Input id="file-upload" type="file" className="sr-only" onChange={handleFileChange} accept="image/jpeg" disabled={isConverting} />
                  </Label>
