...
import { xlsToPdf, XlsToPdfOutput } from '@/ai/flows/xls-to-pdf';
...
export default function XlsToPdfPage() {
  ...
  const [conversionResult, setConversionResult] = useState<XlsToPdfOutput | null>(null);
    ...
    if (selectedFile) {
      if (isFileValid(selectedFile)) {
        ...
        toast({
          ...
          description: 'Please upload a valid Excel file (.xls, .xlsx, .csv, etc.).',
        });
        event.target.value = ''; 
      }
    }
  ...
  const handleDrop = (event: React.DragEvent<HTMLLabelElement>) => {
    ...
    if (droppedFile && isFileValid(droppedFile)) {
      ...
      toast({
        variant: 'destructive',
        title: 'Invalid File Type',
        description: 'Please drop a valid Excel file (.xls, .xlsx, .csv, etc.).',
      });
    }
  };
  ...
  const convertFile = async () => {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        try {
          const result = await xlsToPdf({ xlsDataUri: base64File, fileName: file.name });
        ...
        <main className="lg:col-span-3">
          <Card className="shadow-2xl shadow-primary/10 border-primary/20 bg-card/80 backdrop-blur-sm">
            <CardHeader className="text-center">
              ...
              <CardTitle className="text-3xl font-headline">XLS to PDF Converter</CardTitle>
              <CardDescription className="text-lg">
                Convert your Excel spreadsheets to high-quality PDFs.
              </CardDescription>
            </CardHeader>
          ...
          <article className="mt-16 prose prose-lg dark:prose-invert max-w-none prose-h2:font-headline prose-h2:text-3xl prose-h2:text-primary prose-a:text-primary">
            ...
            <p>Our XLS to PDF converter provides an easy and reliable way to turn your Excel spreadsheets into universal PDF files. PDF is the standard for sharing documents because it preserves formatting across all devices, ensuring your data, charts, and tables look exactly as you intended. This tool handles various Excel formats, including `.xls`, `.xlsx`, and `.csv`.</p>
            <AdBanner type="top-banner" className="my-8"/>
            <h2 id="how-it-works">How Does XLS to PDF Conversion Work?</h2>
