...
import { epubToPdf, EpubToPdfOutput } from '@/ai/flows/epub-to-pdf';
import AdBanner from '@/components/ad-banner';

const acceptedMimeTypes = ['application/epub+zip'];
const acceptedExtensions = ['.epub'];

export default function EpubToPdfPage() {
  const [file, setFile] = useState<File | null>(null);
  const [isConverting, setIsConverting] = useState(false);
  const [conversionResult, setConversionResult] = useState<EpubToPdfOutput | null>(null);
  ...
  const handleDrop = (event: React.DragEvent<HTMLLabelElement>) => {
    ...
    if (droppedFile && isFileValid(droppedFile)) {
      ...
      toast({
        variant: 'destructive',
        title: 'Invalid File Type',
        description: 'Please drop an EPUB file (.epub).',
      });
    }
  };
  ...
  const convertFile = async () => {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        try {
          const result = await epubToPdf({ epubDataUri: base64File, fileName: file.name });
        ...
        <main className="lg:col-span-3">
          <Card className="shadow-2xl shadow-primary/10 border-primary/20 bg-card/80 backdrop-blur-sm">
            <CardHeader className="text-center">
              ...
              <CardTitle className="text-3xl font-headline">EPUB to PDF Converter</CardTitle>
              ...
              {!conversionResult && (
                <div className="space-y-6">
                  ...
                  >
                    ...
                    <Input id="file-upload" type="file" className="sr-only" onChange={handleFileChange} accept=".epub,application/epub+zip" disabled={isConverting} />
                  </Label>
          ...
          <article className="mt-16 prose prose-lg dark:prose-invert max-w-none prose-h2:font-headline prose-h2:text-3xl prose-h2:text-primary prose-a:text-primary">
            <h2 id="about-tool">Convert EPUB E-books to PDF with Ease</h2>
            <p>Our EPUB to PDF converter provides a simple and reliable way to turn your EPUB e-book files into universal PDF documents. While EPUB is a standard for e-readers, PDF offers greater compatibility across all devices, including desktops, laptops, and tablets. This tool is perfect for making your e-book library more accessible and easy to share.</p>
            <AdBanner type="top-banner" className="my-8"/>
            <h2 id="how-it-works">How Does EPUB to PDF Conversion Work?</h2>
            <p>The process is straightforward. When you upload your EPUB document, our system uses an advanced conversion engine to meticulously replicate your e-book's content into a high-quality PDF. The layout, fonts, and images are preserved, ensuring your PDF looks just like the original EPUB file. We prioritize your privacy; all files are handled securely over an encrypted connection and are automatically deleted from our servers after processing.</p>
